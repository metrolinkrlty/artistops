...
export async function POST(req: NextRequest) {
  ...
  if (!agreedToTerms) {
    return NextResponse.json({ ok: false, error: "Please agree to the beta terms to continue" }, { status: 400 });
  }
  ...
  const roleLabel: Record<string, string> = {
    ...
    label: "Label / industry professional", producer: "Producer / engineer",
    educator: "Educator", other: "Other",
  };
  // try/catch (not just .catch) so a stale/missing client — where the model is
  // undefined and .create throws synchronously — can never break the signup.
  try {
    prisma.membershipApplication.create({
      data: { userId: user.id, email, role, referredBy, workLink: workLink || null, goals, catalogSize: catalogSize || null, location: location || null, termsAcceptedAt: new Date() },
    }).catch((e) => console.error("Membership application save failed:", e));
  } catch (e) {
    console.error("Membership application unavailable:", e);
  }
