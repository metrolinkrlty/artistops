...
const highlights: { icon: LucideIcon; title: string; desc: string; soon?: boolean }[] = [
  ...
  { icon: CalendarDays, title: "Gig Procurement", desc: "Book live shows from lead to payday — map fans and tour routes, track every ticket, and settle earnings into revenue.", soon: true },
];
...
export default function LoginView({ tagline }: { tagline: string }) {
  return (
    <div className="min-h-screen bg-[#0f1117] text-white flex flex-col lg:flex-row">
      <div className="lg:w-3/5 p-8 lg:p-14 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-[#2a2d3a]">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-4xl font-bold">ArtistOps<sup className="text-xl font-normal text-[#8b8fa8] ml-0.5">™</sup></h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl">
          {highlights.map((h) => (
            <div key={h.title} className={`relative overflow-hidden flex items-start gap-3 bg-[#1a1d27] border rounded-xl p-3 ${h.soon ? "border-amber-500/40" : "border-[#2a2d3a]"}`}>
              {h.soon && (
                <div className="pointer-events-none absolute -right-12 top-3 rotate-45 bg-amber-500 px-12 py-0.5 text-center text-[9px] font-bold uppercase tracking-wider text-[#0f1117] shadow-md">
                  Coming&nbsp;soon
                </div>
              )}
      ...
      <div className="lg:w-2/5 p-8 lg:p-14 flex items-center justify-center">
        <div className="w-full max-w-sm">
          ...
          <p className="text-center text-[#8b8fa8] text-xs mt-6 leading-relaxed">{tagline}</p>
